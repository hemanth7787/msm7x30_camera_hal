...
/*
 * Contains implementation of an abstract class CameraDevice that defines
 * functionality expected from an  physical camera device:
...
void CameraDevice::setExposureCompensation(const float ev) {
    LOGV("%s", __FUNCTION__);
...
status_t CameraDevice::getCurrentPreviewFrame(void* buffer)
{
    if (!isStarted()) {
        LOGE("%s: Device is not started", __FUNCTION__);
        return EINVAL;
...
status_t CameraDevice::startWorkerThread(bool one_burst)
{
    LOGV("%s", __FUNCTION__);

    if (!isInitialized()) {
        LOGE("%s:  camera device is not initialized", __FUNCTION__);
        return EINVAL;
    }
...
status_t CameraDevice::stopWorkerThread()
{
    LOGV("%s", __FUNCTION__);
...
bool CameraDevice::inWorkerThread()
{
...
status_t CameraDevice::WorkerThread::readyToRun()
{
    LOGV("Starting  camera device worker thread...");
    ...
    if (pipe(thread_fds) == 0) {
        ...
        LOGV(" device's worker thread has been started.");
        return NO_ERROR;
    } else {
...
status_t CameraDevice::WorkerThread::stopThread()
{
    ...
    if (mThreadControl >= 0) {
        ...
        if (wres == sizeof(msg)) {
            ...
            if (res == NO_ERROR) {
                ...
                LOGV(" camera device's worker thread has been stopped.");
            } else {
